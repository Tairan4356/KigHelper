...
class KigvpkParamsManager(context: Context) {
    private val prefs: SharedPreferences =
        context.applicationContext.getSharedPreferences("kigvpk_params", Context.MODE_PRIVATE)
    private val gson = Gson()

    fun get(modelId: String): KigvpkModelParams {
        val json = prefs.getString(modelId, null) ?: return KigvpkModelParams()
        return try {
            gson.fromJson(json, KigvpkModelParams::class.java)
        } catch (_: Exception) {
            KigvpkModelParams()
        }
    }
    ...
    fun save(modelId: String, params: KigvpkModelParams) {
        prefs.edit { putString(modelId, gson.toJson(params)) }
    }

    fun loadDefaults(modelDir: java.io.File, modelId: String): KigvpkModelParams {
        val existing = prefs.getString(modelId, null)
        if (existing != null) return try {
            gson.fromJson(existing, KigvpkModelParams::class.java)
        } catch (_: Exception) {
            KigvpkModelParams()
        }
        ...
        return try {
            val config =
                com.google.gson.JsonParser.parseString(configFile.readText(Charsets.UTF_8)).asJsonObject
