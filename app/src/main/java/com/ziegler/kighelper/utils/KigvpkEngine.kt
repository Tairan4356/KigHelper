...
data class KigvpkVoicePack(
    ...
    companion object {
        ...
        fun fromDir(dir: File): KigvpkVoicePack {
            ...
            val espeakVoice =
                configJson.optJSONObject("espeak")?.optString("voice")?.trim().orEmpty()
            val languageCode =
                configJson.optJSONObject("language")?.optString("code")?.trim().orEmpty()
            val idMap = mutableMapOf<String, List<Int>>()
            configJson.getJSONObject("phoneme_id_map").keys().forEach { key ->
                val values = when (val raw = configJson.getJSONObject("phoneme_id_map").get(key)) {
            ...
            configJson.optJSONObject("phoneme_map")?.keys()?.forEach { key ->
                val values = when (val raw = configJson.getJSONObject("phoneme_map").get(key)) {
...
class EspeakPhonemizer(
    ...
    init {
        if (!EspeakNative.ensureInit(dataDir.absolutePath)) throw IllegalStateException("espeak-ng init failed")
    }
...
class KigvpkTtsEngine(context: Context, val packDir: File) {
    ...
    private val toIds: (String) -> IntArray = run {
        if (voicePack.phonemeType.contains("espeak")) {
            ...
            val voiceName =
                voicePack.espeakVoice.ifBlank { voicePack.languageCode }.ifBlank { "en-us" }
            EspeakPhonemizer(
                dataDir, voiceName, voicePack.phonemeIdMap, voicePack.phonemeMap
            )::toIds
        } else {
    ...
    private val session: OrtSession = env.createSession(
        voicePack.modelPath.absolutePath, OrtSession.SessionOptions().apply {
    ...
    @Volatile
    var sentenceSilenceSec: Float = 0.2f

    fun setSynthesisTuning(
        noiseScale: Float, lengthScale: Float, noiseW: Float, sentenceSilenceSec: Float = 0.2f
    ) {
    ...
    fun synthesize(text: String, sentenceSilenceSec: Float = 0f): FloatArray {
        ...
        val inputName =
            session.inputNames.firstOrNull { it.contains("input") } ?: session.inputNames.first()
        val lenName = session.inputNames.firstOrNull { it.contains("len") || it.contains("length") }
            ?: "${inputName}_length"
        inputs[inputName] = OnnxTensor.createTensor(
            env, LongBuffer.wrap(idLong), longArrayOf(1, idLong.size.toLong())
        )
        inputs[lenName] = OnnxTensor.createTensor(
            env, LongBuffer.wrap(longArrayOf(idLong.size.toLong())), longArrayOf(1)
        )

        val scaleName = session.inputNames.firstOrNull { it.contains("scale") }
        if (scaleName != null) {
            inputs[scaleName] = OnnxTensor.createTensor(
                env, FloatBuffer.wrap(floatArrayOf(noiseScale, noiseW, lengthScale)), longArrayOf(3)
            )
        }
        val sidName = session.inputNames.firstOrNull { it.contains("sid") }
        if (sidName != null) {
            inputs[sidName] =
                OnnxTensor.createTensor(env, LongBuffer.wrap(longArrayOf(0)), longArrayOf(1))
        }
